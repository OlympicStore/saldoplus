...
export const InitialBalance = ({ accounts, onAdd, onUpdate, onDelete }: InitialBalanceProps) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 4 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2 }}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <input
                  ...
                  className="w-28 text-sm font-mono font-semibold text-right bg-background border border-border-subtle rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary tabular-nums"
                />
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-xl p-5 text-center">
        <span className="label-caps text-primary">Saldo Total</span>
        <p className={`text-3xl font-bold font-mono tabular-nums tracking-tight mt-1 ${totalBalance >= 0 ? "text-foreground" : "text-status-negative"}`}>
          {fmt(totalBalance)}
        </p>
        <p className="text-xs text-text-muted mt-2">
          Soma de todas as contas registadas. Este valor será atualizado automaticamente com base nos seus lançamentos.
        </p>
      </div>
    </motion.div>
  );
